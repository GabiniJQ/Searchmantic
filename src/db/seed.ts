...
async function main() {
  ...
  products.forEach(async (product) => {
    ...
    if (!Array.isArray(vector)) throw new Error('Vector is not an array')
    if (vector.length === 0) throw new Error('Vector is empty')
    if (!vector.every((v) => typeof v === 'number'))
      throw new Error('Vector contains non-numbers')
